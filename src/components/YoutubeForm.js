            ...
            {fields.map((field, index) => {
              return (
                <div className='interests-div' key={field.id}>
                  <input type='text' {...register(`interests.${index}.interest`)} />
                  {index > 0 && (<button className='new-btn red' onClick={()=>remove(index)} >remove</button>)}
                </div>
              )
            })}
            <button className='new-btn green' onClick={()=>append({interest:""})} >Add</button>
          </div>
        </div>
